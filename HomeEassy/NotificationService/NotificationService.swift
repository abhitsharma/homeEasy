    ...
    override func didReceive(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void) {
      ...
      self.bestAttemptContent = (request.content.mutableCopy() as? UNMutableNotificationContent)
        ...
        guard let bestAttemptContent = bestAttemptContent, // 1. Make sure bestAttemptContent is not nil
            let apsData = bestAttemptContent.userInfo["fcm_options"] as? [String: Any], // 2. Dig in the payload to get the
            let attachmentURLAsString = apsData["image"] as? String, // 3. The attachment-url
